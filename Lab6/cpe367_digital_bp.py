...
def process_wav(fpath_wav_in,fpath_wav_out):
	...
	Fclp = Fwbp * 0.5 	#digital frequency (samples/cycle)
 
	index = (M-1)//2 	#the range of index
 
	#initialize coefficient list for lowpass
	n=-index
	for i in range(M):
		if n == 0:
			bk[i] = (Fclp) * 2
		else:
			bk[i] = (G * (math.sin(2*math.pi*Fclp*n))) / (math.pi * n)
		n += 1

	n = -index
	#get coefficient list for bandpass and multiply by lowpass to covert
	for i in range(M):
		bk[i] = 2 * math.cos(2*math.pi*Fcbp*n) * bk[i]
		n += 1
	
	#implement hamming window
	for i in range(M):
		w = 0.54 - 0.46 * math.cos((2 * math.pi * i) / (M-1))
		bk[i] = bk[i] * w
